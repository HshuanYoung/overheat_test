...
const XENOBU_FACTION = '瑟诺布';

const hasXenobuGodmarkUnit = (playerState: PlayerState) =>
  playerState.unitZone.some(unit =>
    !!unit &&
    unit.godMark &&
    unit.faction === XENOBU_FACTION
  );
...
const continuous_103090248_power_fixed: CardEffect = {
  id: '103090248_power_fixed',
  type: 'CONTINUOUS',
...
const trigger_103090248_guard: CardEffect = {
  id: '103090248_guard_trigger',
  type: 'TRIGGER',
  triggerLocation: ['UNIT'],
  triggerEvent: 'CARD_ATTACK_DECLARED',
  isGlobal: true,
  isMandatory: true,
  description: '你的单位区有<瑟诺布>神蚀单位，且这个单位为竖置状态时，对手的单位进行攻击时：对方必须攻击这张卡。这场战斗中，你其他的单位不能宣言防御。如果场上有多个单位有此效果，对方选择其中1张作为攻击对象。',
  condition: (_gameState, playerState, instance, event?: GameEvent) => {
    ...
    if (instance.cardlocation !== 'UNIT' || instance.isExhausted || !hasXenobuGodmarkUnit(playerState)) return false;
  ...
  onQueryResolve: async (instance, gameState, _playerState, selections, context) => {
    ...
    if (!ownerState || !hasXenobuGodmarkUnit(ownerState)) return;
 ...
 * Source CardID: 103090248
 * Card2 Row: 357
 * Card Row: 288
 * Source CardNo: ST02-G09
 * Package: ST02(TD)
 * ID Source: card-xlsx
 * Keywords: N/A
 * Card Detail:
 * 和牛头人盾战士效果一致，将神蚀单位要求变为势力‘瑟诺布’
...
const card: Card = {
  ...
  canResetCount: 0,
  effects: [
    continuous_103090248_power_fixed,
    trigger_103090248_guard
  ],
  rarity: 'U',
  availableRarities: ['U'],
  cardPackage: 'BT02',
